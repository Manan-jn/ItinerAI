...
class GroupDetails(BaseModel):
    ...
    group_members: Optional[List[UserProfile]] = Field(
        description="Details of the group members", default=[]
    )
...
class SourceLocation(BaseModel):
    ...
    response_type: Literal["source_point", "text"] = Field(
        description="The type of the response, always 'source_point'",
        default="source_point",
    )
...
class Destination(BaseModel):
    ...
    highlights: str = Field(
        description="Short description highlighting key features", default=""
    )
    ...
    estimated_budget: int = Field(
        description="Estimated budget for the destination in INR", default=0
    )
    suggested_days: int = Field(
        description="Suggested number of days to visit the destination (eg. 2 days, 3 days, 4 days, 5 days, 6 days, 7 days)",
        default=0,
    )
    best_time_to_visit: str = Field(
        description="Best time to visit the destination (eg. January, February, March, April, May, June, July, August, September, October, November, December)",
        default="",
    )
...
class POI(BaseModel):
    ...
    poi_type: Literal[
        "attraction", "restaurant", "hotel", "club", "cafe", "activity", "event"
    ] = Field(description="The type of the place of interest", default="")
    description: str = Field(
        description="Short description highlighting key features", default=""
    )
    rating: str = Field(description="Numerical rating (e.g., 4.5)", default="")
    map_url: str = Field(
        description="The maps URL of the place of interest", default=""
    )
    lat: float = Field(description="The latitude of the place of interest", default=0.0)
    long: float = Field(
        description="The longitude of the place of interest", default=0.0
    )
    photos: List[str] = Field(
        description="The photos of the place of interest", default=[]
    )
    theme: List[str] = Field(
        description="The themes of the place of interest", default=[]
    )


class POISuggestions(BaseModel):
    points_of_interest: List[POI] = Field(
        description="A list of place of interest suggestions", default=[]
    )
...
class RoughTravelDates(BaseModel):
    ...
    timeframe: str = Field(
        description="General timeframe (e.g., 'summer 2025', 'around Christmas', 'early next year')",
        default="",
    )
    flexibility: Literal["very flexible", "somewhat flexible", "fixed dates"] = Field(
        description="How flexible the user is with dates", default="very flexible"
    )
    duration: str = Field(
        description="Approximate trip duration (e.g., '3-5 days', 'a week', '2 weeks')",
        default="",
    )
...
class TravelDates(BaseModel):
    start_date: str = Field(
        description="The start date of the trip in YYYY-MM-DD format (eg. 2025-01-01)",
        default="",
    )
    end_date: str = Field(
        description="The end date of the trip in YYYY-MM-DD format (eg. 2025-01-01)",
        default="",
    )
...
class Budget(BaseModel):
    ...
    overall_estimate: int = Field(
        description="The overall estimate of the budget in INR", default=None
    )
    breakdown: Optional[BudgetBreakdown] = Field(
        description="(Optional) The breakdown of the budget", default=None
    )
...
class MustDoActivity(BaseModel):
    type: Literal[
        "place", "activity", "food", "event", "shopping", "wellness", "transport"
    ] = Field(description="The type of the activity", default="")
    category: str = Field(description="The sub type of the activity", default="")
    name: str = Field(
        description="The specifc name based on the type and category", default=""
    )
    description: str = Field(
        description="The one or two line description about what the user should do based on the type and category",
        default="",
    )
...
class DayPlan(BaseModel):
    ...
    cities: List[Place] = Field(
        description="The cities to visit in the day", default=[]
    )
    must_do_activities: List[MustDoActivity] = Field(
        description="The must do activities in the day", default=[]
    )
...
class Trip(BaseModel):
    ...
    no_of_days: int = Field(
        description="The estimated number of days in the trip", default=0
    )
    estimated_budget: int = Field(
        description="The estimated budget of the trip per person in INR", default=0
    )
    best_time_to_visit: str = Field(
        description="The best time to visit the trip in the year", default=""
    )
    theme: List[str] = Field(description="The themes of the trip", default=[])
    day_wise_plan: List[DayPlan] = Field(
        description="The day wise plan of the trip", default=[]
    )
...
class TripSuggestions(BaseModel):
    ...
    response_type: Literal["trip", "text"] = Field(
        description="The type of the response, always 'trip'", default="trip"
    )
    message: str = Field(description="The message to display to the user", default="")
    trips: List[Trip] = Field(
        description="A list of recommended trip ideas, keep it [] (empty list) if 'response_type' is 'text'",
        default=[],
    )
...
class OnboardingAgent(BaseModel):
    response_type: Literal["text"] = Field(
        description="The type of the response, always 'text'", default="text"
    )
    message: str = Field(description="The message to display to the user", default="")
