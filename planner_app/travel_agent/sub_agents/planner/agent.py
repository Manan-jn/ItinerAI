...
from ...shared_libraries import DestinationIdeas, TravelDates, SourceLocation, Itinerary, Conveyances, StaySuggestions
...
stay_agent = LlmAgent(
    name = "stay_agent",
    description = "An agent that plans the stay for the trip",
    model = "gemini-2.5-flash",
    instruction = prompt.STAY_AGENT_INSTR,
    output_key = "stays",
    output_schema = StaySuggestions,
    disallow_transfer_to_parent=True,
    disallow_transfer_to_peers=True,
    tools=[
        memorize,
