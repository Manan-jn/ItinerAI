...
interface EndResponseLoaderProps {
  showEndLoader: boolean;
  duration?: number; // Duration in milliseconds
}

export function EndResponseLoader({
  showEndLoader,
  duration = 4000,
}: EndResponseLoaderProps) {
  const [currentMessage, setCurrentMessage] = useState("");
  const [isVisible, setIsVisible] = useState(true);
  const [messageIndex, setMessageIndex] = useState(0);

  const loadingMessages = [
    "Let's get your travel dates fixed now",
    "Preparing your date selection",
    "Finalizing your travel timeline",
    "Setting up your calendar",
    "Almost ready for date selection",
  ];
  ...
  }, [showEndLoader, duration, loadingMessages]);

  if (!showEndLoader && !isVisible) {
    return null;
  }
